<!-- <div class="relative">
    <div class="absolute -z-10 rounded-full h-96 w-96 bg-gradient-radial from-mblue via-transparent to-white bottom-0"></div>
</div> -->

<div class="relative overflow-hidden flex flex-col justify-end items-center text-white h-96">
    <div class="flex flex-row gap-10">
        <div>
            <div>MOSAICO 2023</div>
        </div>
        <div>
            <div>FOLLOW OUR SOCIALS:</div>
            <div>
                <img src="" alt="">
                <img src="" alt="">
                <img src="" alt="">
            </div>
        </div>
        <div>
            <div>MADE BY THE BOMBA COLLECTIVE</div>
            <div>CONTACT US</div>
        </div>
    </div>
    <div id="circle" class="-z-10 absolute top-1/2 w-[200%] h-full bg-gradient-radial from-mblue to-transparent"></div>
</div>

<!-- <div class="w-100% h-96 bg-gradient-radial ellipse_at_bottom from-mblue to-transparent to-white">

</div> -->

<style>
    #circle {
        /* position: absolute; */

        background: radial-gradient(50% 50% at 50% 50%, #2E37FF 0%, #FFFFFF 100%);
        }
</style>